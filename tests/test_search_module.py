...
class CheckTextAnalyzeModule(SettingBase):
    ...
    def test_keywords(self):
        response = self.client.get(url_for('rasa_api.keywords')+ "?sender_id=116287698501129823679&keywords=keywords_for_guided_QA,falsk",follow_redirects=True)
    ...
    def test_query_hot_post(self):
        ...
        with open('data/tag-maintenance-module/query_hot_post_output.json', 'r', encoding = 'utf-8') as file:
            output = json.load(file)
        self.assertEqual(response.json,output)
    ...
    def test_query_inner_search(self):
        ...
        response = self.client.post(url_for('post_api.query_inner_search') ,
                                    data = json.dumps(text_input), content_type="application/json")
        with open('data/search-module/query_inner_search_output.json', 'w', encoding = 'utf-8') as file:
